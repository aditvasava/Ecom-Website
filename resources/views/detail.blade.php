...
<html>
  <head>
    ...
    <link rel="stylesheet" href="{{ asset('/css/detailnav.css') }}">
  </head>
  <body>
    <nav>
      ...
      <form action="search">
        ...
        <button type="submit" class="btn btn-info" style="margin-top:-37px;margin-left:380px;width:120px;border-radius:30px;">Search</button>
      </form>
    </div>
    </li>
    ...
    <div class="photwrap">
       <div class="imgdiv">
       <img src="{{$product['gallery']}}" class="detailimag"><br>
       </div>
       <div class="adtoc">
       <form action="/addtocart" method="post">
           @csrf
           <input type="hidden" name="prodid" value={{$product['id']}}>
       <button class="btn btn-primary" style="width:200px;border-radius:5px;position:relative;right:100px;">Add to Cart</button>
       </form>
     </div>
       <div class="descdiv">
       <p style="font-size:20px;">Model Name : {{$product['sname']}}</p>
       <h3><p style="border-bottom:1px solid grey;padding:10px 0px">Price : {{$product['price']}}</p></h3>
       <p style="border-bottom:1px solid grey;padding:10px 0px">Category : {{$product['category']}}</p>
     <div class="descriptionwr">
     <p style="border-bottom:1px solid grey;overflow:auto;padding:10px 0px">Details :-<br>{{$product['description']}}</p>
   </div>
   </div>
   </div>
   <br><br>
